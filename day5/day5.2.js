...
fs.readFile(path.join(__dirname, 'day5.input'), 'utf8', (err, data) => {
    ...
    for (let pair = 0; pair < seeds.length / 2; pair++) {
        const seed = seeds[pair * 2];
        const length = seeds[(pair * 2) + 1];

        console.log('processing seed:', seed, 'length:', length, 'total seeds: ', seeds.length / 2);

        for (let i = seed; i < seed + length; i++) {
            const location = processSeed(i);
